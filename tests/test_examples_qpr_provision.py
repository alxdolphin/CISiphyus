...
import qpr  # noqa: E402
from qpr import (  # noqa: E402
    _cisiphyus_cmd,
...
def test_prefetch_skips_when_fresh(monkeypatch, tmp_path: Path) -> None:
    ...
    result = qpr.maybe_prefetch_student_metrics(["provision-batch"], run=fake_run)
...
def test_prefetch_refreshes_stale_workbook(monkeypatch, tmp_path: Path) -> None:
    ...
    result = qpr.maybe_prefetch_student_metrics(
        ["provision-batch"],
        require_fresh=True,
        run=fake_run,
    )
...
def test_resolve_paths_use_bundled_fixtures() -> None:
    assert qpr.resolve_metric_workbook(FIXTURE_METRICS) == FIXTURE_METRICS.resolve()
    assert qpr.resolve_template_path(FIXTURE_TEMPLATE) == FIXTURE_TEMPLATE.resolve()
...
def test_provision_requires_bundled_qpr_tools(monkeypatch, tmp_path: Path) -> None:
    ...
    with patch.object(qpr, "load_qpr_tools", side_effect=FileNotFoundError("missing bundled qpr_tools")):
        code = qpr.main(["/tmp/out", "--grading-period", "2.0", "--no-site-staff-filter"])
    assert code == 1
